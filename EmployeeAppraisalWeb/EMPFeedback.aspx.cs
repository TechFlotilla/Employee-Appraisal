...
public partial class EMPFeedback : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                var client = (from ob in DC.tblProjects
                              where ob.ManagerID == Convert.ToUInt32(Session["EmpID"])
                              select ob).Single();
                ...
                if (emp > 0)
                {
                    ...
                    tblClient org = (from ob in DC.tblClients
                                     where ob.ClientID == client.ClientID
                                     select ob).Single();
                    ...
                    txtOrgn.Text = org.CompanyName;
                     ...
                     ddProduct.DataSource = BindEMPProject(org.ClientID,client.ProjectID);
                    //ddProduct.DataSource = objFeedBack.BindNewProject(client.ProjectID);
    ...
    public IQueryable<tblProject> BindEMPProject(int ClientID,int projectID)
    {
        var DC = new DataClassesDataContext();
        var Data = from obj in DC.tblProjects
                   where obj.ClientID==ClientID && obj.ProjectID== projectID
                   select obj;
        return Data;
    }
